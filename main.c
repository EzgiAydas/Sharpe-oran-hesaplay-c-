#include <stdio.h> // C dilindeki temel girdi ve çýktý iþlemlerini gerçekleþtiren fonksiyonlarýn tanýmlarýný içerir.

// Girdi tamponunu temizleyen fonksiyon
void clearInputBuffer(void) {
    int c;
    while ((c = getchar()) != '\n' && c != EOF) {
        // Kullanýcý 'Enter' tuþuna basana kadar veya dosya sonuna gelene kadar tüm karakterleri okur ve yok sayar.
		 }
}
...
int main(void) {
    ...
    if (scanf("%lf", &yatirim_getirisi) != 1) { // Okunan deðerin basariyla atanip atanmadigini kontrol eder.
    ...
    // Negatif veya sýfýr deðer kontrolü
    if (standart_sapma <= 0) {
